...
export default function DashboardAttention({ rows, onOpen }: {
  ...
  return (
    <section aria-labelledby="dashboard-attention-heading" className="col-span-full w-full min-w-0 overflow-hidden rounded-2xl border border-[#D3E1EC] bg-white">
      <div className="flex min-h-16 items-center justify-between gap-4 border-b border-[#E1EBF2] px-5 py-4">
      ...
      {rows.length === 0 ? (
        ...
        <ul className="divide-y divide-[#E1EBF2]">
          {rows.slice(0, 5).map(row => {
            ...
            return (
              <li key={`${row.category}:${row.id}`} className="grid grid-cols-[32px_minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1 px-5 py-3 md:grid-cols-[32px_minmax(140px,1fr)_minmax(0,2fr)_auto]">
